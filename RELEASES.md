...
## [v2.4.0] - 2026-07-12
### Summary
* **[토스 키오스크 옵션 아키텍처 및 관리자 레시피 매핑 통합 개편]**
  - **본사 DB 중심 옵션매핑 단일화**:
    - 개별 가맹점의 구글 시트 연동 없이, 본사 마스터 DB(`web_option_mappings` 테이블)를 단일 진실원(Source of Truth)으로 삼아 전체 매장 옵션 레시피 코드를 일원화 통제하도록 아키텍처를 전면 간소화했습니다.
  - **관리자 페이지 내 표준 옵션 매핑 관리 UI 구현**:
    - 관리자 연동 설정 하단에 표준 레시피 코드(48종)를 직접 생성, 조회, 수정 및 DB 저장이 가능한 인라인 그리드 에디터를 신규 구현하였습니다.
  - **대시보드 기본 온도(HOT/ICE) 필터 기능 탑재**:
    - 점주 대시보드에 대표 온도 설정을 추가하여 지정된 온도(HOT/ICE) 기준 상품 코드와 음료 이미지만 ZIP 및 엑셀에 패키징되도록 필터링을 최적화했습니다. (아이스 전용 음료는 예외적으로 포함)
  - **연동 데이터 실시간 검증 로그 추가**:
    - 다운로드 패널 내에 생성된 일괄등록 엑셀 상품의 행 수와 ZIP 내 이미지 파일 개수를 실시간으로 대조해주는 교차 검증 로그창을 추가하여 연동 신뢰도를 개선했습니다.
  - **부원두(사용원두2) 매핑 가이드 연동 누락 버그 해결**:
    - 대시보드 설정에 맞추어 디카페인/프리미엄 원두 옵션 변경 시의 대표 음료 레시피 가이드라인이 엑셀에 동적으로 2개 행(독립 옵션 기준)으로 추가 포함되도록 보완했습니다.

| Domain Path | Modified Files | Detailed Changes (What & Why) | DB Schema Changes |
| :--- | :--- | :--- | :--- |
| `frontend/` | `components/membership_comp_toss.tsx`, `components/admin/admin_page_test_validator.tsx` | 가맹점 구글 시트 쓰기 버튼 제거, 기본 온도 필터 및 실시간 검증창 추가, 관리자 페이지 내 옵션 매핑 에디터 UI 및 조회/저장 연동 | 없음 |
| `backend/` | `server/routes/admin/menu.ts` | GET `/api/menu-items/option-mappings` API 추가 및 표준 레시피 저장 로직 동기화 | 없음 |

## [v2.3.1] - 2026-07-07
### Summary
* **[신규 Q&A 게시글 추가]**
  - **월구독료 및 가입비 세금계산서 발행 안내**:
    - ID 119번으로 "월구독료와 가입비는 세금계산서 발행이 되나요?" 글을 추가하였습니다.
    - `local_posts_qna.json`, `seeder.ts`, `schema-setup.ts` 에 반영 완료했습니다.

| Domain Path | Modified Files | Detailed Changes (What & Why) | DB Schema Changes |
| :--- | :--- | :--- | :--- |
| `backend/` | `local_posts_qna.json`, `server/db/seeder.ts`, `server/db/schema-setup.ts` | 월구독료 및 가입비 세금계산서 발행 여부에 대한 Q&A 게시글 ID 119 추가, 시더 및 DB 스키마 셋업 반영 | 없음 |
