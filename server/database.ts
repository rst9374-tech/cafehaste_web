...
class SupabaseRpcPool {
  ...
  async query(sql: string, params: any[] = []) {
    ...
    try {
      ...
      console.error(`[Supabase Query Failed] operation failed on production database:`, err.message);
      throw err;
    }
  }
...
export async function getDbPool(forceReconnect = false) {
  ...
  if (process.env.NODE_ENV !== 'production' && !hasCloudEnv) {
    throw new Error('[DB Initialization] Local Simulator Fallback is disabled. Please configure SUPABASE_URL and SUPABASE_KEY.');
  }
  ...
  if (forceReconnect || !pool) {
    ...
    try {
      ...
      console.error(`[DB Connection Error] Supabase Rest RPC connection failed: ${rpcErr.message || rpcErr}`);
      if (!pool) {
