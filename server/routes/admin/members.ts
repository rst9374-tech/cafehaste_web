...
router.get('/api/registered-members', async (req, res) => {
  try {
    ...
    // Fetch all licenses to dynamically compute approval status
    let licenses: any[] = [];
    try {
      const [licRows]: any = await dbPool.query('SELECT store_id, is_approved, license_end_date FROM web_store_licenses');
      licenses = licRows || [];
    } catch (e) {
      console.error('[Status Sync Warning] Failed to fetch web_store_licenses:', e);
    }

    const filteredRows = rows.filter((row: any) => {
      const phoneVal = (row.phone || '').trim();
      const hasPhone = phoneVal !== '' && phoneVal !== '없음' && phoneVal !== '기재 안 됨' && phoneVal !== '010-0000-0000';
      const isKakao = row.signup_path === '카톡주입';
      return hasPhone && !isKakao;
    });
    ...
    const mappedRows = filteredRows.map((row: any) => {
      ...
      const lic = storeCode ? licenses.find((l: any) => (l.store_id || '').trim() === storeCode) : null;
      
      let approvalStatus = row.approval_status || '요청';
      
      if (lic) {
        const isApproved = lic.is_approved === true || Number(lic.is_approved) === 1;
        const isSuspended = lic.is_approved === false || Number(lic.is_approved) === 0;
        const isPending = Number(lic.is_approved) === 2;
        
        const expireDate = lic.license_end_date;
        const isNotExpired = expireDate ? (new Date(`${expireDate}T23:59:59`).getTime() >= today.getTime()) : false;
        
        if (isPending) {
          approvalStatus = '인증 대기';
        } else if (isSuspended) {
          approvalStatus = '가동 정지';
        } else if (isApproved && !isNotExpired) {
          approvalStatus = '기간 만료';
        } else if (isApproved && isNotExpired) {
...
router.delete('/api/registered-members/:id', async (req, res) => {
  ...
  try {
    ...
    // 1. Fetch store_code and associated files to delete
    try {
      const [memberRows]: any = await dbPool.query(
        'SELECT store_code, business_cert_path, agreement_document_url FROM web_membership_users WHERE id = ?',
        [numericId]
      );
      
      if (memberRows && memberRows.length > 0) {
        const storeCode = memberRows[0].store_code;
        const certPath = memberRows[0].business_cert_path;
        const agreementDocumentUrl = memberRows[0].agreement_document_url;
        
        // Delete associated certificate file on disk
        ...
        // Delete associated agreement document file on disk
        if (agreementDocumentUrl && agreementDocumentUrl !== '없음' && typeof agreementDocumentUrl === 'string') {
          const cleanAgreementPath = agreementDocumentUrl.replace(/^\//, '');
          const fullAgreementPath = path.join(process.cwd(), cleanAgreementPath);
          if (fs.existsSync(fullAgreementPath)) {
            fs.unlinkSync(fullAgreementPath);
            console.log(`[File System delete] Successfully deleted agreement document file on disk: ${fullAgreementPath}`);
          }
        }
        ...
        if (storeCode) {
          await dbPool.query(
            'DELETE FROM web_store_licenses WHERE store_id = ?',
            [storeCode]
          );
          console.log(`[License delete] Successfully deleted associated license for store_code: ${storeCode}`);
        }
      }
    } catch (innerErr) {
      console.warn('[Warning] Failed to delete associated files or license:', innerErr);
    }
    ...
    res.json({
      success: true,
      message: '성공적으로 회원 데이터, 연동 라이선스 및 첨부파일이 영구 삭제되었습니다.'
    });
  } catch (err: any) {
...
router.get('/api/admin-master-bulk', async (req, res) => {
  try {
    ...
    if (dbPool.isFallback) {
      ...
      const filteredBMembers = bMembers.filter((row: any) => {
        const phoneVal = (row.phone || '').trim();
        const hasPhone = phoneVal !== '' && phoneVal !== '없음' && phoneVal !== '기재 안 됨' && phoneVal !== '010-0000-0000';
        const isKakao = (row.signupPath === '카톡주입' || row.signup_path === '카톡주입');
        return hasPhone && !isKakao;
      });
      members = filteredBMembers.map((row: any) => ({
        id: `DB-HST-${row.id}`,
        ...
        storeType: row.store_type || row.storeType || '멤버십',
      ...
      const filteredMembRows = membRows.filter((row: any) => {
        const phoneVal = (row.phone || '').trim();
        const hasPhone = phoneVal !== '' && phoneVal !== '없음' && phoneVal !== '기재 안 됨' && phoneVal !== '010-0000-0000';
        const isKakao = row.signup_path === '카톡주입';
        return hasPhone && !isKakao;
      ...
      members = filteredMembRows.map((row: any) => ({
        id: `DB-HST-${row.id}`,
        storeName: row.store_name,
        storeCode: row.store_code || '',
        ownerName: row.owner_name,
        ...
        joinDateTime: row.created_at ? new Date(row.created_at).toISOString() : new Date().toISOString(),
        approvalStatus: row.approval_status || '요청',
        storeType: row.store_type || 'MEMBERSHIP',
        businessNumber: row.business_number || '',
        businessCertPath: row.business_cert_path || '',
        signupPath: row.signup_path || '맴버십가입신청',
        source: 'CLOUD_SQL'
  ...
  try {
    ...
    if (dbPool.isFallback) {
      ...
      if (found) {
        ...
        targetStoreType = found.storeType || '멤버십';
      }
    } else {
      ...
      if (mRows && mRows.length > 0) {
        ...
        targetStoreType = mRows[0].store_type || 'MEMBERSHIP';
      }
    }
    ...
    const targetStoreGrade = (targetStoreType === 'EXECUTIVE' || targetStoreType === 'HASTE_MEMBERSHIP') ? 'PREMIUM' : 'STANDARD';
