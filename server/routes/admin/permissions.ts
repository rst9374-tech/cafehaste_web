  ...
  try {
    ...
    if (dbPool.isFallback) {
      ...
      const [rows]: any = await dbPool.query(
        'SELECT id, grade_type, category_key, can_read, can_write FROM web_board_grade_permissions ORDER BY id ASC'
      );
  ...
  try {
    ...
    if (dbPool.isFallback) {
      ...
      try {
        ...
        await conn.query('DELETE FROM web_board_grade_permissions');
        ...
        for (const perm of permissions) {
          ...
          await conn.query(
            'INSERT INTO web_board_grade_permissions (grade_type, category_key, can_read, can_write) VALUES (?, ?, ?, ?)',
            [gType, cKey, readVal, writeVal]
          );
        }
