...
router.post('/api/consultations/move-to-member', async (req, res) => {
  ...
  try {
    ...
    const finalStoreType = consultation.has_store === '있음' ? '프리미엄' : '멤버십';
    ...
    for (const memberId of memberIds) {
      ...
      if (dbPool.isFallback) {
        ...
        if (licRows && licRows.length > 0) {
          await dbPool.query('UPDATE web_store_licenses SET is_approved = false WHERE store_id = ?', [targetStoreId]);
          updatedCount++;
        } else {
    ...
    for (const memberId of memberIds) {
      ...
      if (dbPool.isFallback) {
        ...
        if (mRows && mRows.length > 0) {
          storeCode = mRows[0].store_code || '';
          storeName = mRows[0].store_name || '';
          storeType = mRows[0].store_type || '멤버십';
        }
      }
      ...
      const endDateStr = endDate.toISOString().split('T')[0];
      const isApprovedVal = true; // 인증만료/종료임박 등도 기본 승인 가동 씰 상태에서 만료일자로 판정함

      const targetStoreGrade = (storeType === 'EXECUTIVE' || storeType === 'HASTE_MEMBERSHIP') ? 'PREMIUM' : 'STANDARD';
