...
import { invalidateBoardCache } from '../../db/board_io';
...
router.get('/api/posts', async (req, res) => {
  ...
  try {
    ...
    // Find requester's grade
    let userGrade = '일반';
    if (memberId) {
      const [mRows]: any = await dbPool.query('SELECT store_type FROM web_membership_users WHERE id = ?', [memberId]);
      if (mRows && mRows.length > 0) {
        userGrade = mRows[0].store_type || '일반';
      } else {
        const [kRows]: any = await dbPool.query('SELECT store_type FROM web_kakao_members WHERE id = ?', [memberId]);
        if (kRows && kRows.length > 0) {
          userGrade = kRows[0].store_type || '일반';
        }
      }
    }

    // Query denied categories (can_list = 0) for this user grade
    let deniedCategories: string[] = [];
    if (!isAdmin) {
      const [deniedRows]: any = await dbPool.query(
        'SELECT category_key FROM web_grade_permissions WHERE grade_type = ? AND can_list = 0',
        [userGrade]
      );
      deniedCategories = deniedRows.map((r: any) => r.category_key);
    }
    ...
    if (!category && !searchKwd) {
      ...
      queryStr = `
        WITH RankedPosts AS (
          ...
          WHERE P.title NOT LIKE '%테스트%'
            ...
            AND P.category NOT LIKE '%Test%'
            ${deniedCategories.length > 0 ? 'AND P.category NOT IN (?)' : ''}
        )
        ...
        LIMIT ? OFFSET ?
      `;
      queryParams = deniedCategories.length > 0
        ? [memberId, memberId, deniedCategories, limit, offset]
        : [memberId, memberId, limit, offset];
      ...
      `;
      countParams = deniedCategories.length > 0 ? [deniedCategories] : [];
    } else {
      ...
      if (searchKwd) {
        if (searchType === 'title') {
          whereClauses.push(`P.title ILIKE ?`);
          queryParams.push(`%${searchKwd}%`);
        } else if (searchType === 'content') {
          whereClauses.push(`P.content ILIKE ?`);
          queryParams.push(`%${searchKwd}%`);
        } else {
          whereClauses.push(`(P.title ILIKE ? OR P.content ILIKE ?)`);
          queryParams.push(`%${searchKwd}%`, `%${searchKwd}%`);
        }
      }
      ...
      if (deniedCategories.length > 0) {
        whereClauses.push(`P.category NOT IN (?)`);
        queryParams.push(deniedCategories);
      }
      ...
      if (whereClauses.length > 0) {
        ...
        if (searchKwd) {
          if (searchType === 'title') {
            countWhereClauses.push(`P.title ILIKE ?`);
            countParams.push(`%${searchKwd}%`);
          } else if (searchType === 'content') {
            countWhereClauses.push(`P.content ILIKE ?`);
            countParams.push(`%${searchKwd}%`);
          } else {
            countWhereClauses.push(`(P.title ILIKE ? OR P.content ILIKE ?)`);
            countParams.push(`%${searchKwd}%`, `%${searchKwd}%`);
          }
        }
        if (category) {
        ...
        if (deniedCategories.length > 0) {
          countWhereClauses.push(`P.category NOT IN (?)`);
          countParams.push(deniedCategories);
        }
...
router.get('/api/posts/:id', async (req, res) => {
  ...
  try {
    ...
    // Check grade-level permissions (can_read and can_list)
    if (!isAdmin) {
      let userGrade = '일반';
      if (memberId) {
        const [mRows]: any = await dbPool.query('SELECT store_type FROM web_membership_users WHERE id = ?', [memberId]);
      ...
      // Query grade permission for category
      const [permRows]: any = await dbPool.query(
        'SELECT can_read, can_list FROM web_grade_permissions WHERE grade_type = ? AND category_key = ?',
        [userGrade, post.category]
      );
      
      const canRead = permRows.length > 0 ? (permRows[0].can_read === 1) : true;
      const canList = permRows.length > 0 ? (permRows[0].can_list === 1) : true;

      if (!canRead || !canList) {
        return res.status(403).json({ success: false, message: '해당 카테고리의 상세 내용은 멤버십 회원만 조회할 수 있습니다.' });
      }
    }

    // Increment view count since permission check succeeded
...
router.post('/api/posts', async (req, res) => {
  const { memberId, title, content, isSecret, files, category, isNotice } = req.body;
  ...
  try {
    ...
    const [result]: any = await dbPool.query(
      'INSERT INTO web_board_posts (member_id, title, content, is_secret, category, is_notice) VALUES (?, ?, ?, ?, ?, ?)',
      [memberId, title, sanitizedContent, !!isSecret, category || 'Q&A', !!isNotice]
    );
...
router.put('/api/posts/:id', async (req, res) => {
  const { id } = req.params;
  const { title, content, isSecret, files, deletedFileIds, category, isNotice } = req.body;
  ...
  try {
    const dbPool = await getDbPool();
    await dbPool.query(
      'UPDATE web_board_posts SET title = ?, content = ?, is_secret = ?, category = ?, is_notice = ? WHERE id = ?',
      [title, sanitizedContent, !!isSecret, category || 'Q&A', !!isNotice, id]
    );
...
// 5.6 Bulk update notice status for posts
router.post('/api/posts/bulk-notice', async (req, res) => {
  const { postIds, isNotice } = req.body;
  if (!postIds || !Array.isArray(postIds) || postIds.length === 0) {
    return res.status(400).json({ success: false, message: '공지 설정할 게시글 ID 목록이 필요합니다.' });
  }

  try {
    const dbPool = await getDbPool();
    await dbPool.query(
      'UPDATE web_board_posts SET is_notice = ? WHERE id IN (?)',
      [!!isNotice, postIds]
    );
    invalidateBoardCache();
    return res.json({ success: true, message: '선택한 게시글들의 공지 설정이 완료되었습니다.' });
  } catch (err: any) {
    console.error('Bulk notice setting error:', err);
    return res.status(500).json({ success: false, message: '일괄 공지 설정 중 오류가 발생했습니다: ' + err.message });
  }
});

// 5.7 Bulk update categories for posts
router.post('/api/posts/bulk-category', async (req, res) => {
  const { postIds, category } = req.body;
  if (!postIds || !Array.isArray(postIds) || postIds.length === 0 || !category) {
    return res.status(400).json({ success: false, message: '이동할 게시글 ID 목록과 대상 카테고리가 필요합니다.' });
  }

  try {
    const dbPool = await getDbPool();
    await dbPool.query(
      'UPDATE web_board_posts SET category = ? WHERE id IN (?)',
      [category, postIds]
    );
    invalidateBoardCache();
    return res.json({ success: true, message: `선택한 ${postIds.length}개 게시글의 카테고리가 [${category}](으)로 일괄 변경되었습니다.` });
  } catch (err: any) {
    console.error('Bulk update categories error:', err);
    return res.status(500).json({ success: false, message: '일괄 카테고리 이동 처리 중 오류가 발생했습니다: ' + err.message });
  }
});
