...
function getExtendedLicenseDate(months: number = 1, date: Date = new Date()): { startStr: string; endStr: string } {
  ...
  const mValue = 1; // Always force 1 month
  const bufferDays = 0; // Always 0 buffer days
  ...
  try {
    ...
    if (dbPool.isFallback) {
      ...
      if (licenseRows && licenseRows.length > 0) {
        await dbPool.query(
          "UPDATE web_store_licenses SET license_start_date = ?, license_end_date = ?, is_approved = true WHERE store_id = ?",
          [startStr, endStr, storeCode]
        );
      } else {
        await dbPool.query(
          "INSERT INTO web_store_licenses (store_name, store_id, license_start_date, license_end_date, is_approved, store_grade) VALUES (?, ?, ?, ?, true, 'PREMIUM')",
