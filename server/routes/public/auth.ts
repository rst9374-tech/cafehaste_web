...
router.post('/api/owner-registrations/bulk-test', async (req, res) => {
  try {
    ...
    for (let i = 0; i < count; i++) {
      ...
      // 라이선스 승인 여부 매핑
      // '인증 완료' -> 1, '인증 대기' -> 2, '가동 정지중' -> 0
      let isApprovedVal = 1;
      if (randomStatus === '인증 대기') isApprovedVal = 2;
      else if (randomStatus === '가동 정지중') isApprovedVal = 0;
      ...
      if (!dbPool.isFallback) {
        try {
          await dbPool.query(
            'INSERT INTO web_membership_users (store_name, store_code, owner_name, phone, email, address, content, approval_status, store_type, business_number, business_cert_path, signup_path, password, role) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)',
            [storeName, storeCode, ownerName, memberItem.phone, memberItem.email, memberItem.address, memberItem.content, randomStatus, '일반', memberItem.business_number, '없음', '멤버십가입신청', hashedPassword, 'USER']
          );
