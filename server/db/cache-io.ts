...
export const BACKUP_SETTINGS_FILE = path.join(process.cwd(), 'local_settings.json');
...
export function readBackupSettings(): any[] {
  if (process.env.NODE_ENV === 'production' && dbCache.settings !== null) return dbCache.settings;
  dbCache.settings = readJsonFile<any[]>(BACKUP_SETTINGS_FILE, [
    { setting_key: 'draft_random_show', setting_value: 'false' }
  ]);
  return dbCache.settings;
}

export function writeBackupSettings(settings: any[]) {
  dbCache.settings = settings;
  writeJsonFile(BACKUP_SETTINGS_FILE, settings);
}
