export const boardApiClient = {
  fetchPosts: async (page: number, keyword: string, category: string, memberId: any, isAdmin: boolean) => {
    const catParam = category === '전체' ? '' : category;
    const res = await fetch(`/api/posts?page=${page}&limit=100&keyword=${encodeURIComponent(keyword)}&category=${encodeURIComponent(catParam)}&memberId=${memberId}&isAdmin=${isAdmin}`);
