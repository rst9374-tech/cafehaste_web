...
export function useAppController(
  ...
  useEffect(() => {
    const fetchFreshMenuItemsForSignature = async () => {
      try {
        const resMenuItems = await fetch('/api/menu-items-all/raw');
        if (resMenuItems.ok) {
          const data = await resMenuItems.json();
          if (data.success && Array.isArray(data.items)) {
            const signatureList = data.items.filter((item: any) => (item.is_signature === true || item.is_signature === 1) && item.bean_type !== 'D' && item.bean_type !== 'P');
