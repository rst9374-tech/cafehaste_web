...
export const HasteAdmin: React.FC<HasteAdminProps> = ({ 
  ...
  const [subTabPages, setSubTabPages] = useState<'FILMS' | 'DRAFTS' | 'INQUIRY' | 'MENU'>('FILMS');
      ...
      {currentMainTab === 'PAGES' && (
        <>
          ...
          <div className="flex flex-wrap bg-stone-100 p-1 rounded-xl border border-stone-200 gap-1 mb-4 select-none w-fit">
            ...
            <button
              onClick={() => setSubTabPages('MENU')}
              className={`px-3.5 py-1.5 text-xs font-bold transition-all rounded-lg cursor-pointer flex items-center gap-1.5 ${ subTabPages === 'MENU' ? 'bg-stone-900 text-white shadow-sm font-extrabold' : 'text-stone-500 hover:text-stone-900 hover:bg-stone-200/40' }`}
            >
              <Database size={12} />
              <span>메뉴 게시판</span>
            </button>
