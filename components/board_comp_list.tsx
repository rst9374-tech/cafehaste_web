...
interface BoardCompListProps {
  ...
  onMovePostsCategory?: (postIds: number[], category: string) => void;
...
export const BoardCompList: React.FC<BoardCompListProps> = ({
  ...
  const [subCategory, setSubCategory] = React.useState<'all' | 'as'>('all');
  const [knowhowSub, setKnowhowSub] = React.useState<'all' | 'machine' | 'ice' | 'kiosk' | 'run' | 'etc'>('all');
  ...
  // Reset subCategory when main category changes (posts list changes)
  React.useEffect(() => {
    setSubCategory('all');
    setKnowhowSub('all');
  }, [posts]);
  ...
  const isArchiveCategory = selectedCategory === '자료실';
  const isKnowhowCategory = selectedCategory === '노하우팁';

  return (
    <div className="flex flex-col w-full -mt-2 md:-mt-3 gap-1.5">
      {/* 자료실 서브 카테고리 탭 (자료실 > A/S업체정보) */}
      {isArchiveCategory && (
        <div className="flex items-center gap-2 pb-2 mb-0.5 border-b border-stone-200 select-none">
          <button
            type="button"
            onClick={() => setSubCategory('all')}
            className={`px-3.5 py-1.5 rounded-xl text-xs font-black transition-all cursor-pointer ${subCategory === 'all' ? 'bg-stone-900 text-[#C5A059] shadow-xs' : 'bg-stone-100 text-stone-600 hover:bg-stone-200'}`}
          >
            <span>전체 자료</span>
          </button>
          <button
            type="button"
            onClick={() => setSubCategory('as')}
            className={`px-3.5 py-1.5 rounded-xl text-xs font-black transition-all cursor-pointer ${subCategory === 'as' ? 'bg-amber-700 text-white shadow-xs' : 'bg-stone-100 text-stone-600 hover:bg-stone-200'}`}
          >
            <span>A/S업체정보</span>
          </button>
        </div>
      ...
      {/* 노하우팁 서브 카테고리 탭 (노하우팁 > 소분류 대괄호 필터) */}
      {isKnowhowCategory && (
        <div className="flex items-center gap-1.5 pb-2 mb-0.5 border-b border-stone-200 select-none overflow-x-auto scrollbar-none">
          <button
            type="button"
            onClick={() => setKnowhowSub('all')}
            className={`px-3 py-1.5 rounded-xl text-xs font-black transition-all cursor-pointer whitespace-nowrap ${knowhowSub === 'all' ? 'bg-stone-900 text-[#C5A059] shadow-xs' : 'bg-stone-100 text-stone-600 hover:bg-stone-200'}`}
          >
            <span>전체 노하우</span>
          </button>
          <button
            type="button"
            onClick={() => setKnowhowSub('machine')}
            className={`px-3 py-1.5 rounded-xl text-xs font-black transition-all cursor-pointer whitespace-nowrap ${knowhowSub === 'machine' ? 'bg-amber-700 text-white shadow-xs' : 'bg-stone-100 text-stone-600 hover:bg-stone-200'}`}
          >
            <span>커피머신</span>
          </button>
          <button
            type="button"
            onClick={() => setKnowhowSub('ice')}
            className={`px-3 py-1.5 rounded-xl text-xs font-black transition-all cursor-pointer whitespace-nowrap ${knowhowSub === 'ice' ? 'bg-blue-600 text-white shadow-xs' : 'bg-stone-100 text-stone-600 hover:bg-stone-200'}`}
          >
            <span>제빙기</span>
          </button>
          <button
            type="button"
            onClick={() => setKnowhowSub('kiosk')}
            className={`px-3 py-1.5 rounded-xl text-xs font-black transition-all cursor-pointer whitespace-nowrap ${knowhowSub === 'kiosk' ? 'bg-purple-650 text-white shadow-xs' : 'bg-stone-100 text-stone-600 hover:bg-stone-200'}`}
          >
            <span>키오스크</span>
          </button>
          <button
            type="button"
            onClick={() => setKnowhowSub('run')}
            className={`px-3 py-1.5 rounded-xl text-xs font-black transition-all cursor-pointer whitespace-nowrap ${knowhowSub === 'run' ? 'bg-emerald-700 text-white shadow-xs' : 'bg-stone-100 text-stone-600 hover:bg-stone-200'}`}
          >
            <span>매장운영</span>
          </button>
          <button
            type="button"
            onClick={() => setKnowhowSub('etc')}
            className={`px-3 py-1.5 rounded-xl text-xs font-black transition-all cursor-pointer whitespace-nowrap ${knowhowSub === 'etc' ? 'bg-stone-500 text-white shadow-xs' : 'bg-stone-100 text-stone-600 hover:bg-stone-200'}`}
          >
            <span>기타 노하우</span>
          </button>
        </div>
        ...
        <div className="flex items-center justify-between bg-stone-50/70 border border-stone-200/60 rounded-2xl p-2 px-3 select-none mt-0">
          ...
          {selectedIds.length > 0 && (
            <div className="flex items-center gap-2 flex-wrap sm:flex-nowrap">
              {isAdmin && onMovePostsCategory && (
                <div className="flex items-center bg-white border border-stone-300 rounded-xl p-0.5 shadow-2xs shrink-0">
                  <select
                    onChange={(e) => {
                      const targetCat = e.target.value;
                      if (targetCat) {
                        onMovePostsCategory(selectedIds, targetCat);
                        setSelectedIds([]);
                        e.target.value = "";
                      }
                    }}
                    className="text-[10px] font-bold text-stone-700 bg-white outline-none cursor-pointer p-1 py-0.5"
                    defaultValue=""
                  >
                    <option value="" disabled>카테고리 이동</option>
                    {['헤이스트소식', '노하우팁', '자료실', '핵심정보', '헤이스트멤버십전용', 'Q&A'].map((cat) => (
      ...
      <div className="flex flex-col divide-y divide-stone-300">
        {(() => {
          ...
          if (isArchiveCategory && subCategory === 'as') {
            displayPosts = displayPosts.filter(p => p.title.includes('[A/S업체정보]') || p.title.includes('A/S'));
          }
          if (isKnowhowCategory && knowhowSub !== 'all') {
            displayPosts = displayPosts.filter(p => {
              const title = p.title || '';
              if (knowhowSub === 'machine') return title.includes('[커피머신]') || title.includes('[머신]');
              if (knowhowSub === 'ice') return title.includes('[제빙기]');
              if (knowhowSub === 'kiosk') return title.includes('[키오스크]') || title.includes('[포스');
              if (knowhowSub === 'run') return title.includes('[매장운영]') || title.includes('[매장') || title.includes('[디저트') || title.includes('[기기운영]') || title.includes('[매장 공간 관리]') || title.includes('[디저트 관리]');
              if (knowhowSub === 'etc') {
                return !(
                  title.includes('[커피머신]') || title.includes('[머신]') ||
                  title.includes('[제빙기]') ||
                  title.includes('[키오스크]') || title.includes('[포스') ||
                  title.includes('[매장운영]') || title.includes('[매장') || title.includes('[디저트') || title.includes('[기기운영]') || title.includes('[매장 공간 관리]') || title.includes('[디저트 관리]')
                );
              }
              return true;
            });
            ...
            <div className="flex items-center gap-2 flex-wrap sm:flex-nowrap">
              {isAdmin && onMovePostsCategory && (
                <div className="flex items-center bg-white border border-stone-300 rounded-xl p-0.5 shadow-2xs shrink-0">
                  ...
                  >
                    <option value="" disabled>카테고리 이동</option>
                    {['헤이스트소식', '노하우팁', '장비운영', '자료실', '레시피', '핵심정보', '헤이스트멤버십전용', 'Q&A', 'TEST'].map((cat) => (
                      <option key={cat} value={cat}>
                        {cat === '헤이스트소식' ? '소식' : cat === '노하우팁' ? '노하우팁' : cat}
                      </option>
                    ))}
