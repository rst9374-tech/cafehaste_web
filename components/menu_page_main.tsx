...
// Import our new submodules
import { MenuCustomizeModal } from './menu_modal_customize';
import { MenuDrawerCart } from './menu_drawer_cart';
import { useMenuLoad } from './menu_hook_load';
import type { MenuItem, CartItem } from './menu_types';
import { getDrinkSvg, handleImageError } from './menu_types';

// Re-export types and utilities from menu_types for backward compatibility
export type { MenuItem, CartItem } from './menu_types';
export { HASTE_ITEMS, HASTE_FALLBACK_IMAGES, handleImageError, getDrinkExtractionTime, getDrinkSvg } from './menu_types';


interface HasteMenuProps {
  cart: CartItem[];
...
export const HasteMenu: React.FC<HasteMenuProps> = ({ 
  cart, 
  setCart, 
  defaultPickupBranch, 
  onCheckout,
  ...
  // Load via our dedicated custom hook
  const { categories, menuItems, setMenuItems, isLoading } = useMenuLoad();
  ...
  const searchFilteredItems = [...menuItems].sort((a, b) => {
    const isSigA = a.isSignature === 1 || a.isSignature === true || (a as any).is_signature === 1 || (a as any).is_signature === true ? 1 : 0;
    const isSigB = b.isSignature === 1 || b.isSignature === true || (b as any).is_signature === 1 || (b as any).is_signature === true ? 1 : 0;
    return isSigB - isSigA;
  });

  const filteredItems = searchFilteredItems.filter(item => {
    if (selectedCategory === 'ALL') return true;
    if (selectedCategory === 'SIGNATURE') {
      return item.isSignature === 1 || item.isSignature === true || (item as any).is_signature === 1 || (item as any).is_signature === true;
    }
  ...
  return (
    <div id="haste-menu-view-root" className="pt-1 md:pt-2 pb-4 md:pb-10 bg-[var(--haste-body-bg)] relative">
      
      {/* Category selector capsules */}
      <section className="max-w-5xl mx-auto px-3 md:px-6 mb-2 md:mb-4 pb-1 flex justify-start md:justify-between items-center overflow-x-auto md:overflow-x-visible md:flex-wrap gap-2 scrollbar-none" style={{ WebkitOverflowScrolling: 'touch' }}>
        <div className="flex gap-1.5 flex-nowrap md:flex-wrap items-center">
          <button
            id="menu-cat-btn-ALL"
      ...
      <section className="max-w-5xl mx-auto px-3 sm:px-6 mb-4 md:mb-10">
        {isLoading ? (
          <div className="w-full">
            ...
            <div className="grid grid-cols-2 xs:grid-cols-3 sm:grid-cols-4 md:grid-cols-4 lg:grid-cols-5 gap-2 sm:gap-3">
            ...
            {categories.map((catGroup) => {
              ...
              return (
                <div key={catGroup.id} className="w-full">
                  ...
                  <div className="grid grid-cols-2 xs:grid-cols-3 sm:grid-cols-4 md:grid-cols-4 lg:grid-cols-5 gap-1.5 sm:gap-1.5 md:gap-2">
                    {groupItems.map(item => {
                      ...
                      return (
                        <div
                          ...
                          className={`bg-white rounded-xl border overflow-hidden shadow-sm hover:shadow-md transition-all duration-300 flex flex-col group cursor-pointer ${ isSig ? 'border-[#C5A059]/45 bg-stone-50/10 hover:border-[#C5A059]' : 'border-stone-200/60 hover:border-[#C5A059]/50' }`}
                        >
          ...
          <div className="w-full">
            <div className="mb-1.5 md:mb-3 pb-0.5 md:pb-1 pl-1">
              ...
              <h3 className="font-sans text-sm sm:text-2xl font-bold text-stone-900 uppercase mb-2">
                {selectedCategory === 'SIGNATURE' ? '⭐ 시그니처 추천 메뉴' : (categories.find(c => c.id === selectedCategory)?.name || selectedCategory)}
              </h3>
              <p className="text-stone-650 font-sans text-[10px] sm:text-xs font-light mt-0.5 leading-relaxed">
