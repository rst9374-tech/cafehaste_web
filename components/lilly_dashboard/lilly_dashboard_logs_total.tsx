...
export function LogTotal({ storeCode }: { storeCode: string }) {
  ...
  const fetchLogs = async () => {
    try {
      const response = await fetch(`/api/remote/logs?storeCode=${storeCode}&name=total`);
  ...
  return (
    <div className="rounded-xl border border-[#27272A]/60 bg-[#141414] shadow-sm p-4.5 text-left font-sans flex flex-col gap-3">
      ...
      {isLoading ? (
        <div className="h-[360px] flex items-center justify-center text-xs text-[#71717A] font-mono">LOADING TOTAL LOG LINES...</div>
      ) : (
