...
export function LogTask({ storeCode }: { storeCode: string }) {
  ...
  const fetchLogs = async () => {
    try {
      const response = await fetch(`/api/remote/logs?storeCode=${storeCode}&name=task`);
    ...
    <div className="rounded-xl border border-[#27272A]/60 bg-[#141414] shadow-sm p-4.5 text-left font-sans flex flex-col gap-3">
      <div className="flex justify-between items-center text-[10px] font-semibold text-[#71717A] uppercase tracking-wider">
        <span className="flex items-center gap-1.5">
          ...
          ORDER SCHEDULER LOGS (주문 및 스케줄러 로그)
        </span>
        <button
          onClick={fetchLogs}
      ...
      {isLoading ? (
        <div className="h-[360px] flex items-center justify-center text-xs text-[#71717A] font-mono">LOADING SCHEDULER LOGS...</div>
      ) : (
