  ...
  return (
    <div className="w-full max-w-5xl mx-auto font-sans px-3 md:px-6">
      <div className="bg-white border border-stone-200 flex flex-col rounded-2xl md:rounded-[32px] p-3.5 md:p-8 gap-3.5 md:gap-6 shadow-sm md:shadow-md">
        ...
        <div className="w-full flex justify-end pb-2 border-b border-stone-100">
          <div className="flex flex-wrap bg-stone-50 p-0.5 rounded-lg gap-0.5 select-none font-sans justify-end w-fit">
            ...
            {(isApproved || user?.role === 'ADMIN' || user?.username === 'admin') && (
              <button 
                type="button" 
                onClick={() => navigateTo('CONTROL')} 
                className="flex items-center gap-1 px-2 py-1 text-[9px] font-bold bg-[#C5A059]/20 hover:bg-[#C5A059]/35 border border-[#C5A059]/40 text-stone-900 rounded-md transition-all active:scale-95 cursor-pointer"
              >
                <Sliders size={9} className="text-[#C5A059]" />
                <span>대시보드</span>
              </button>
            )}
            <button 
      ...
      <HasteAgreementModal
        ...
        businessNumber={user.businessNumber || user.business_number || ''}
