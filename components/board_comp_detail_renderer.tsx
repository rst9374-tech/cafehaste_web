...
const parseInlineHtmlTags = (inputStr: string): React.ReactNode[] => {
  if (!inputStr) return [];
  const tagRegex = /(<(?:span|u|del|s|strike|div)(?:\s+style=["'](?:[^"']*;)*\s*[^"';]+["']|\s+style=&quot;[^&]+&quot;)*>.*?<\/span>|<u[^>]*>.*?<\/u>|<del[^>]*>.*?<\/del>|<s[^>]*>.*?<\/s>|<strike[^>]*>.*?<\/strike>|<div style=["'][^"']*["']>.*?<\/div>)/gi;
  const parts = inputStr.split(tagRegex);
  ...
  parts.forEach((part, index) => {
    if (index % 2 === 1) {
      ...
      const tagNameMatch = cleanTag.match(/^<([a-z1-6]+)/i);
      if (!tagNameMatch) {
        elements.push(part);
        return;
      }
      ...
      const innerText = cleanTag.replace(/^<[a-z1-6]+[^>]*>/i, '').replace(/<\/span>$|<\/u>$|<\/del>$|<\/s>$|<\/strike>$|<\/div>$/i, '');
      
      const styleMatch = cleanTag.match(/style=["']([^"']+)["']/i);
      ...
      if (styleMatch) {
        ...
        declarations.forEach(decl => {
          ...
          if (colonIndex !== -1) {
            ...
            if (key === 'font-size') styleObj.fontSize = val;
            else if (key === 'font-family') styleObj.fontFamily = val.replace(/^['"]|['"]$/g, '');
            else if (key === 'line-height') styleObj.lineHeight = val;
            else if (key === 'text-align') styleObj.textAlign = val as any;
      ...
      if (tagName === 'span') {
        elements.push(
          <span key={`span-${index}`} style={styleObj}>
            {parseInlineHtmlTags(innerText)}
          </span>
        );
      } else if (tagName === 'u') {
        elements.push(
          <u key={`u-${index}`} style={styleObj}>
            {parseInlineHtmlTags(innerText)}
          </u>
        );
      } else if (['del', 's', 'strike'].includes(tagName)) {
        elements.push(
          <del key={`del-${index}`} style={styleObj}>
            {parseInlineHtmlTags(innerText)}
          </del>
        );
      } else if (tagName === 'div') {
        elements.push(
          <div key={`div-${index}`} style={styleObj}>
            {parseInlineHtmlTags(innerText)}
          </div>
        );
      } else {
    ...
    if (i % 2 === 1) {
      ...
      codeParts.forEach((codePart, j) => {
        if (j % 2 === 1) {
          ...
          const normalizedCodePart = codePart.replace(/<(b|strong)[^>]*>([\s\S]*?)<\/\1>/gi, '**$2**');
          const boldParts = normalizedCodePart.split(/\*\*([^*]+)\*\*/g);
          boldParts.forEach((subPart, k) => {
            if (k % 2 === 1) {
              ...
              result.push(...parseInlineHtmlTags(subPart));
            }
          });
        }
      });
