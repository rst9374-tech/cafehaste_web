...
export const useBoardState = ({ loggedUser, setActivePlayFilm }: UseBoardStateProps) => {
  ...
  const [activeSearchType, setActiveSearchType] = useState<'all' | 'title' | 'content'>('all');
  ...
  useEffect(() => {
    fetchPosts();
  }, [currentPage, activeKwd, activeSearchType, selectedCategory]);
  ...
  const checkListPermissionForCategory = (catName: string): boolean => {
    if (isAdmin) return true;
    const userGrade = loggedUser ? (loggedUser.store_type || loggedUser.storeType || '일반') : '일반';
    const cat = catName === '전체' ? 'Q&A' : catName;
    const perm = gradePermissions.find((p: any) => p.gradeType === userGrade && p.categoryKey === cat);
    return perm ? perm.canList === 1 : true;
  };

  const checkReadPermissionForCategory = (catName: string): boolean => {
    if (isAdmin) return true;
    const userGrade = loggedUser ? (loggedUser.store_type || loggedUser.storeType || '일반') : '일반';
    const cat = catName === '전체' ? 'Q&A' : catName;
    const perm = gradePermissions.find((p: any) => p.gradeType === userGrade && p.categoryKey === cat);
    return perm ? perm.canRead === 1 : true;
  };
  ...
  const loadSharedPostDetail = async (postId: number) => {
    ...
    try {
      ...
      if (data.success) {
        ...
        // Final sanity check for grade-based read/list permission on fetched post
        if (!isAdmin && (!checkReadPermissionForCategory(post.category) || !checkListPermissionForCategory(post.category))) {
          alert('해당 카테고리의 상세 내용은 멤버십 회원만 조회할 수 있습니다.');
          return;
        }
      ...
      alert(e.message || '게시글 상세 조회 권한이 없거나 오류가 발생했습니다.');
    } finally {
  ...
  const fetchPosts = async () => {
    setIsLoading(true);
    try {
      const data = await boardApiClient.fetchPosts(currentPage, activeKwd, selectedCategory, memberId, isAdmin, activeSearchType);
      if (data.success) {
        setPosts(data.posts || []);
  ...
  const handleSelectPost = async (post: any) => {
    ...
    // Check list/read permissions client-side
    if (!checkReadPermissionForCategory(post.category) || !checkListPermissionForCategory(post.category)) {
      alert('해당 카테고리의 상세 내용은 멤버십 회원만 조회할 수 있습니다.');
      return;
    }
    ...
    try {
      ...
      alert(e.message || '게시글 상세 조회 중 오류가 발생했습니다.');
    } finally {
      setIsDetailLoading(false);
    }
  };
  ...
  const handleSavePost = async (e: React.FormEvent, overrideContent?: string) => {
    e.preventDefault();
    let targetTitle = writeTitle.trim();
    const targetContent = (overrideContent !== undefined ? overrideContent : writeContent).trim();
    ...
    try {
      if (isEditing && editingPostId) {
        const res = await fetch(`/api/posts/${editingPostId}`, {
          ...
          body: JSON.stringify({
            title: targetTitle,
            content: overrideContent !== undefined ? overrideContent : writeContent,
  ...
  const handleMovePostsCategory = (postIds: number[], category: string) => {
    setConfirmModal({
      message: `선택하신 ${postIds.length}개의 문의글의 카테고리를 [${category}](으)로 일괄 변경하시겠습니까?`,
      onConfirm: async () => {
        setIsLoading(true);
        try {
          const data = await boardApiClient.movePostsCategoryBulk(postIds, category);
          if (data.success) {
            setSelectedPost(null);
            setViewState('LIST');
            fetchPosts();
          } else {
            alert(data.message || '일괄 카테고리 이동에 실패했습니다.');
          }
        } catch (e: any) {
          console.error('Failed to bulk move category:', e);
          alert('일괄 이동 중 오류가 발생했습니다: ' + e.message);
        } finally {
          setIsLoading(false);
        }
      }
  ...
  return {
    ...
    checkListPermissionForCategory,
    checkReadPermissionForCategory,
    fetchPosts,
    handleSelectPost,
    handleFileDrop,
    handleFileSelect,
