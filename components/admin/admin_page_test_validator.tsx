...
export const AdminPageTestValidator: React.FC = () => {
  ...
  const [simSizeGroup, setSimSizeGroup] = useState('미니벤티');
  ...
  const handleSizeGroupChange = (size: string) => {
    setSimSizeGroup(size);
    const filtered = products.filter(p => p.sizeGroup === size);
    if (filtered.length > 0) {
      setSimProductV4Code(filtered[0].v4Code);
    }
  };
    ...
    reader.onload = (evt) => {
      try {
        ...
        workbook.SheetNames.forEach(sheetName => {
          ...
          json.forEach(row => {
            const categoryKey = Object.keys(row).find(k => k.includes('대분류')) || '대분류';
            const v4CodeKey = Object.keys(row).find(k => k.includes('최종') || k.includes('연동')) || '최종 연동코드';
            const oldCodeKey = Object.keys(row).find(k => k.includes('상품코드') || k.includes('원래')) || '상품코드';
            const nameKey = Object.keys(row).find(k => k.includes('상품명') || k.includes('이름')) || '상품명';
            const priceKey = Object.keys(row).find(k => k.includes('판매가') || k.includes('가격')) || '판매가';

            const category = row[categoryKey] ? String(row[categoryKey]).trim() : '기타';
            const v4Code = row[v4CodeKey] ? String(row[v4CodeKey]).trim() : '';
            const oldCode = row[oldCodeKey] ? String(row[oldCodeKey]).trim() : '';
            const name = row[nameKey] ? String(row[nameKey]).trim() : '';
            const price = parseInt(row[priceKey]) || 3000;
        ...
        if (extractedProducts.length === 0) {
          showToast('올바른 템플릿 형식의 엑셀 파일을 업로드해 주세요.', 'error');
          return;
        }

        setProducts(extractedProducts);
        setMappingSource('excel');
        localStorage.setItem('haste_kiosk_products', JSON.stringify(extractedProducts));
        localStorage.setItem('haste_kiosk_products_source', 'excel');
        
        setSimSizeGroup(extractedProducts[0].sizeGroup);
        setSimProductV4Code(extractedProducts[0].v4Code);
        setSimCart([]);
        
        showToast('엑셀 파일이 업로드되어 동기화되었습니다.', 'success');
      } catch (err: any) {
        showToast('엑셀 파싱 중 오류가 발생했습니다.', 'error');
      }
    };
  ...
  const handleDownloadTemplate = () => {
    try {
      ...
      sheetNames.forEach(sheetName => {
        const filtered = products.filter(p => p.sizeGroup === sheetName);
      ...
      XLSX.writeFile(workbook, '통합키오스크_표준코드_매핑_v4.xlsx');
      showToast('통합레시피 매핑 엑셀 다운로드 완료.', 'success');
    } catch (err: any) {
  ...
  const handleGoogleSheetSync = async () => {
    ...
    try {
      ...
      setSimSizeGroup(allExtractedProducts[0].sizeGroup);
  ...
  const testAccounts = useMemo(() => {
    ...
    return targets.map(storeId => {
      ...
      if (found) {
        ...
        if (storeId === 'store123456') { storeName = '강남본점'; statusLabel = '인증 완료'; statusColor = 'text-emerald-700 bg-emerald-50 border-emerald-200'; expireDate = '2026-12-31'; }
        else if (storeId === 'store123457') { storeName = '역삼지점'; storeGrade = 'STANDARD'; statusLabel = '인증 완료'; statusColor = 'text-emerald-700 bg-emerald-50 border-emerald-200'; expireDate = '2026-08-15'; }
        else if (storeId === 'store123458') { storeName = '홍대입구역점'; statusLabel = '기간 만료'; statusColor = 'text-amber-700 bg-amber-50 border-amber-200'; expireDate = '2026-05-01'; }
        else if (storeId === 'store123459') { storeName = '부산서면점'; statusLabel = '가동 정지중'; statusColor = 'text-rose-700 bg-rose-50 border-rose-200'; expireDate = '2027-03-01'; }
        else if (storeId === 'store123460') { storeName = '신규가맹점'; statusLabel = '인증 대기'; statusColor = 'text-blue-700 bg-blue-50 border-blue-200'; expireDate = '2027-06-01'; }
      }
          ...
          <div className="lg:col-span-5 space-y-6">
            ...
            <div className="bg-white border border-stone-200 rounded-3xl p-5 shadow-sm space-y-4">
              ...
              <div className="space-y-4">
                ...
                <div className="grid grid-cols-2 gap-3">
                  <div className="space-y-1">
                    <label className="text-xs font-bold text-stone-500 uppercase tracking-wider block">로컬 수신 주소 (URL)</label>
                    <input
                      type="text"
                      value={kioskUrl}
                      readOnly
                      placeholder="https://local.cafehaste.com:8080/api/order"
                      className="w-full text-xs font-mono border border-stone-200 rounded-xl p-2.5 bg-stone-100/80 text-stone-500 select-all focus:outline-none transition-all cursor-not-allowed"
                    />
                  </div>
                  <div className="space-y-1">
                    <label className="text-xs font-bold text-stone-500 uppercase tracking-wider block">인증 토큰 (X-Haste-API-Key)</label>
                    <input
                      type="text"
                      value={kioskToken}
                      readOnly
                ...
                <div className="bg-stone-50 border border-stone-200/60 p-3.5 rounded-2xl flex flex-wrap gap-2 items-center justify-between">
                  <div className="flex gap-2">
                    <a
                      href="/Integrated_Recipe_Standard_Map_v4_1.xlsx"
                      download="통합키오스크_표준코드_매핑_v4.1.xlsx"
                      className="bg-emerald-600 hover:bg-emerald-700 text-white text-xs font-bold px-3 py-1.5 rounded-lg transition-all cursor-pointer flex items-center gap-1 shadow-sm"
                    >
                      <Database size={11} />
                      통합키오스크_표준코드 다운
                    </a>
                    {mappingSource !== 'default' && (
                      <button
                        type="button"
                        className="bg-rose-50 border border-rose-200 hover:bg-rose-100 text-rose-600 text-xs font-bold px-3 py-1.5 rounded-lg transition-all cursor-pointer"
                  ...
                  <div className="relative">
                    ...
                    />
                    <label
                      htmlFor="excel-upload-web"
                      className="bg-indigo-600 hover:bg-indigo-700 text-white text-xs font-bold px-3 py-1.5 rounded-lg transition-all cursor-pointer flex items-center gap-1"
                    >
            ...
            <div className="bg-white border border-stone-200 rounded-3xl p-5 shadow-sm space-y-4">
              ...
              <div className="space-y-4">
                ...
                <div className="grid grid-cols-2 md:grid-cols-4 gap-2 text-left">
                  <div className="space-y-1">
                    <label className="text-xs font-bold text-stone-500 uppercase tracking-wider block">컵 분류</label>
                    <select
                      className="w-full text-sm border border-stone-200 rounded-xl p-2 bg-stone-50 focus:outline-none cursor-pointer"
                      value={simSizeGroup}
                      onChange={(e) => handleSizeGroupChange(e.target.value)}
                    >
                      {Array.from(new Set(products.map(p => p.sizeGroup))).map(size => (
                  ...
                  <div className="space-y-1 col-span-2">
                    ...
                    >
                      {products.filter(p => p.sizeGroup === simSizeGroup).map(prod => (
                  ...
                  <div className="space-y-1">
                    ...
                    <div className="flex gap-1.5">
                      <input
                        type="number"
                        className="w-12 text-center text-sm border border-stone-200 rounded-xl p-2 bg-stone-50 focus:outline-none"
                        value={simQuantity}
                      ...
                      <button
                        type="button"
                        className="flex-1 bg-emerald-600 hover:bg-emerald-700 text-white text-sm font-bold py-2 rounded-xl transition-all shadow-sm cursor-pointer"
                  ...
                  return (
                    <div className="bg-amber-50/30 border border-amber-200/50 p-3.5 rounded-2xl text-xs text-stone-700 space-y-1.5 text-left">
                      ...
                      <div className="grid grid-cols-2 gap-x-4 gap-y-1 font-mono text-xs">
                        ...
                        <div className="col-span-2"><span className="text-stone-400">상품명:</span> <span className="text-stone-800">{selectedProd.name}</span></div>
                      </div>
                    </div>
                  );
                })()}
            ...
            <div className="bg-white border border-stone-200 rounded-3xl p-5 shadow-sm space-y-4 text-left">
              <div className="flex justify-between items-center border-b border-stone-100 pb-2.5 flex-wrap gap-2">
                <h4 className="text-sm font-bold text-stone-900 flex items-center gap-1.5">
                  <FileJson size={16} className="text-[#C5A059]" />
                  <span>토스 키오스크 로컬 연동 API 규격서 (v2.3.0)</span>
                </h4>
                <div className="flex gap-1.5 items-center flex-wrap">
                  <a
                    href="/Toss_Kiosk_Integration_Guide_v2.3.0.md"
                    download="Toss_Kiosk_Integration_Guide_v2.3.0.md"
                    className="text-[10px] bg-indigo-50 hover:bg-indigo-100 text-indigo-600 border border-indigo-200 px-2 py-1.5 rounded-lg font-bold transition-all flex items-center gap-1 cursor-pointer"
                  >
                    <BookOpen size={11} />
                    <span>연동 가이드 (.md)</span>
