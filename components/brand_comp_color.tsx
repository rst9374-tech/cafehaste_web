...
import logoPng1 from '../src/assets/images/1.png';
import logoPng2 from '../src/assets/images/2.png';
import logoPng3 from '../src/assets/images/3.png';
import logoPng4 from '../src/assets/images/4.png';

export const HasteColor: React.FC = () => {
  const [selectedLogoIdx, setSelectedLogoIdx] = useState(0);

  const logos = [
    { src: logoSvg1, pngSrc: logoPng1, name: 'haste_logo_horizontal_black.svg', pngName: 'haste_logo_horizontal_black.png', label: '가로형 블랙' },
    { src: logoSvg3, pngSrc: logoPng3, name: 'haste_logo_horizontal_white.svg', pngName: 'haste_logo_horizontal_white.png', label: '가로형 화이트' },
    { src: logoSvg2, pngSrc: logoPng2, name: 'haste_logo_vertical_black.svg', pngName: 'haste_logo_vertical_black.png', label: '세로형 블랙' },
    { src: logoSvg4, pngSrc: logoPng4, name: 'haste_logo_vertical_white.svg', pngName: 'haste_logo_vertical_white.png', label: '세로형 화이트' }
  ];

  const handleDownloadSvg = async (svgUrl: string, filename: string) => {
    try {
  ...
  const handleDownloadPng = async (pngUrl: string, filename: string) => {
    try {
    ...
    <section id="brand-color" className="max-w-5xl mx-auto scroll-mt-28 bg-white border border-stone-300 rounded-[24px] shadow-md relative overflow-hidden flex flex-col md:flex-row gap-6 items-center mb-4 p-4 md:mb-36 md:p-14">
      ...
      <div className="md:w-auto flex justify-center items-center relative z-10 w-full shrink-0">
        ...
        >
          ...
          <div className="w-full px-5 mt-3 mb-2 z-10 flex gap-2.5 justify-center">
            <button
              type="button"
              onClick={() => handleDownloadSvg(logos[selectedLogoIdx].src, logos[selectedLogoIdx].name)}
              className="flex-1 py-3 px-3 rounded-xl bg-stone-900 border border-stone-850 hover:bg-stone-800 text-white font-sans font-medium text-[11px] tracking-wide shadow-md hover:shadow-lg transition-all duration-300 flex items-center justify-center gap-1.5 group/btn active:scale-[0.98] cursor-pointer"
            >
              <ArrowDown size={13} className="text-[#ffbd59] group-hover/btn:translate-y-0.5 transition-transform duration-300" />
              <span>SVG 다운로드</span>
            </button>
            <button
              type="button"
              onClick={() => handleDownloadPng(logos[selectedLogoIdx].pngSrc, logos[selectedLogoIdx].pngName)}
              className="flex-1 py-3 px-3 rounded-xl bg-[#ffbd59] border border-[#e2a849] hover:bg-[#ffc875] text-stone-900 font-sans font-bold text-[11px] tracking-wide shadow-md hover:shadow-lg transition-all duration-300 flex items-center justify-center gap-1.5 group/btn active:scale-[0.98] cursor-pointer"
            >
              <ArrowDown size={13} className="text-stone-900 group-hover/btn:translate-y-0.5 transition-transform duration-300" />
              <span>PNG 다운로드</span>
            </button>
          </div>
        </motion.div>
