...
app.get('/uploads/*relativePath', (req, res) => {
  ...
  // 1. Redirect to Supabase if it matches a known file structure pattern
  const redirectUrl = getSupabaseRedirectUrl(relativePath);
  if (redirectUrl) {
    return res.redirect(redirectUrl);
  }

  // 2. Serve the premium fallback image when any other upload file is missing!
  const fallbackPaths = [
